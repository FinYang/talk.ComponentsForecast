...
p_forec <- data_cluster %>% 
  slice_min(..id) %>% 
  slice_min(.id) %>% 
  slice_min(name) %>% 
  select(Month, component, value) %>%
  mutate(component = gsub("Component", "", component) %>% 
           factor(levels = 1:10)) %>% 
  filter(as.numeric(component) <= 3) %>% 
...
ggsave("plot/p_forec.pdf", p_forec,
       height = unit(3.5, "cm"),
       width = unit(10 / 1.4, "cm")
)
```
Forecastable components [@Goerg2013-yu] maximise \alert{forecastability}, finding linear combinations with \alert{most regular patterns}
...
1. Adding back trend and seasonality. Reversing Box Cox transformation.
...
Compare Average ranks of mRMSSE in cross-validations [@Koning2005-ch]
...
mcb_pdata_top3 %>%
  ...
  ggtitle(latex2exp::TeX(r"(\{Component\}\ \{Model\}\{$\delta$\}\_\{No. comp\})"))
...
* Random forest works best for longer term forecast. Linear models work best for short term forecast.
...
#| out-width: 100%
#| out-height: 100%
comp <- qs::qread("data/pca.qs")

comp %>%
  mutate(component = gsub("Component", "", component) %>%
           factor(levels = paste0(1:10)),
         type = recode(type,
                       fc = "Forecast",
                       real = "Component")) %>%
  ggplot(aes(x = Month, y = value, group = ..id, colour = type)) +
  geom_line(alpha = 0.3) +
